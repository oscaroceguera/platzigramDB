...
test('like image', async t => {
  ...
  t.is(typeof db.likeImage, 'function', 'likeImage is afunction')
...
test('save user', async t => {
  ...
  t.is(typeof db.saveUser, 'function', 'save is a function')

  let user = fixtures.getUser()
  let plaintPassword = user.password
  let created = await db.saveUser(user)

  t.is(user.username, created.username)
  t.is(user.email, created.email)
  t.is(user.name, created.name)
  t.is(utils.encrypt(plaintPassword), created.password)
  t.is(typeof created.id, 'string')
  t.truthy(created.createdAt)
})
...
test('authenticate user', async t => {
  ...
  let success = await db.authenticate(user.plainPassword)
  t.true(success)
